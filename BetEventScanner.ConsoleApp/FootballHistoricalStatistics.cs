...
namespace BetEventScanner.ConsoleApp
{
    public class FootballHistoricalStatistics
    {
        ...
        private readonly IDictionary<string, Func<FootballResult, bool>> _scoreRules = new Dictionary
            <string, Func<FootballResult, bool>>
        {
        ...
        public void ProcessMatch(FootballResult matchResult)
        {
            foreach (var rule in _scoreRules.Where(rule => rule.Value.Invoke(matchResult)))
            {
        ...
        public void ProcessMatches(IEnumerable<FootballResult> matchResults)
        {
